...
func _ready():
	pass
	#get_node("StaticBody2D").get_node("CollisionPolygon2D").shape = get_node("CollisionShape2D").shape
	#get_node("StaticBody2D").get_node("CollisionPolygon2D").position = get_node("CollisionShape2D").position
...
#			else:
#				get_node("StaticBody2D").get_node("CollisionPolygon").disabled = false


func _on_body_exited(body: Node2D) -> void:
	#if !broken:
	if body is RigidBody2D:
		get_node("StaticBody2D").get_node("CollisionPolygon").disabled = true
